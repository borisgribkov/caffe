...
namespace caffe {
  ...
  void AddAngularMarginLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  template <typename Dtype>
  void AddAngularMarginLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                                    const vector<Blob<Dtype>*>& top) {
    if(top[0] != bottom[0]) top[0]->ReshapeLike(*bottom[0]);
  }

template <typename Dtype>
void AddAngularMarginLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                                  const vector<Blob<Dtype>*>& top) {
  ...
  Dtype threshold = std::cos(M_PI - angle_);
  Dtype mm = std::sin(M_PI - angle_) * angle_;
...
template <typename Dtype>
void AddAngularMarginLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(AddAngularMarginLayer);
#endif

INSTANTIATE_CLASS(AddAngularMarginLayer);
REGISTER_LAYER_CLASS(AddAngularMargin);
